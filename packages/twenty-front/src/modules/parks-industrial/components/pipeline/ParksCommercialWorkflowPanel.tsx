...
import { formatParksNumber, formatParksUsd } from '@/parks-industrial/utils/parks-format.util';
import {
    ...
    >
      ...
      >        {tourNaves.length === 0 ? (
          ...
          <>
            ...
            <StyledPreviewValue>
              {t`Renta mensual`}: USD{' '}
              {(rentaPreview ?? liveRentaPreview ?? 0).toLocaleString('en-US')}
              {quotedNave ? (
                <>
                  {' '}
                  · {quotedNave.identificador} ·{' '}
                  {formatParksNumber(m2Ofertados)} m² × ${precioPorM2}
                </>
              ) : null}
            </StyledPreviewValue>
              ...
              >
                <StyledParksSelect
                  ...
                  onChange={(event) =>
                    setHojaForm((current) =>
                      current
                        ? { ...current, tipoContrato: event.target.value }
                        : current,
                    )
                  }
                >
              ...
              >
                <StyledParksSelect
                  ...
                  onChange={(event) =>
                    setHojaForm((current) =>
                      current
                        ? { ...current, esquemaComision: event.target.value }
                        : current,
                    )
                  }
                >
