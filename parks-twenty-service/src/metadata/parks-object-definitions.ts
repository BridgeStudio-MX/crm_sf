...
export const PARKS_OBJECT_DEFINITIONS: ParksObjectDefinition[] = [
  ...
  {
    nameSingular: 'empresaBroker',
    namePlural: 'empresasBroker',
    labelSingular: 'Empresa de Brokers',
    labelPlural: 'Empresas de Brokers',
    ...
    fields: [
      { name: 'nombre', label: 'Nombre', type: 'TEXT', isNullable: false },
      {
        ...
        label: 'Contacto principal / Gerente',
        type: 'TEXT',
      },
      ...
      {
        name: 'comisionPct',
        label: 'Comisión acordada % (legacy)',
        type: 'NUMBER',
      },
      {
        name: 'comisionPctNuevo',
        label: 'Comisión % — nuevo / construida',
        type: 'NUMBER',
      },
      {
        name: 'comisionPctPreventa',
        label: 'Comisión % — preventa / por construir',
        type: 'NUMBER',
      },
      {
        name: 'comisionPctRenovacion',
      ...
      },
      {
        ...
        label: 'Clasificación',
        type: 'SELECT',
        options: buildSelectOptions(['Top 10', 'No top 10']),
      },
      {
        name: 'clasificacionHistorialJson',
        label: 'Historial de tier (JSON)',
        type: 'TEXT',
      },
      {
        name: 'sectores',
        label: 'Sectores / tipo de clientes',
        type: 'TEXT',
      },
      {
        name: 'zonasOperacion',
  ...
  },
  {
    ...
    fields: [
      ...
      { name: 'folio', label: 'Folio del deal', type: 'TEXT' },
  ...
  },
  {
    ...
    fields: [
      ...
      { name: 'slaPausado', label: 'SLA pausado', type: 'BOOLEAN' },
      { name: 'fechaPausaSla', label: 'Fecha pausa SLA', type: 'DATE' },
      { name: 'motivoPausa', label: 'Motivo pausa', type: 'TEXT' },
      { name: 'diasPausados', label: 'Días pausados', type: 'NUMBER' },
      { name: 'versionActual', label: 'Versión actual', type: 'NUMBER' },
      {
        name: 'requiereConvenioConfidencialidad',
        label: 'Requiere NDA',
        type: 'BOOLEAN',
      },
      { name: 'ndaFirmado', label: 'NDA firmado', type: 'BOOLEAN' },
  ...
  {
    nameSingular: 'actaRestitucion',
    namePlural: 'actasRestitucion',
    labelSingular: 'Acta de Restitución',
    labelPlural: 'Actas de Restitución',
    icon: 'IconDoorExit',
    skipNameField: true,
    labelIdentifierFieldName: 'referencia',
    fields: [
      { name: 'referencia', label: 'Referencia', type: 'TEXT', isNullable: false },
      { name: 'fechaSalidaCliente', label: 'Fecha salida cliente', type: 'DATE' },
      { name: 'fechaRecepcionActa', label: 'Fecha recepción acta', type: 'DATE' },
      { name: 'diasRetrasoActa', label: 'Días retraso acta', type: 'NUMBER' },
      {
        name: 'estadoNave',
        label: 'Estado nave',
        type: 'SELECT',
      ...
      { name: 'descripcionDesperfectos', label: 'Descripción desperfectos', type: 'TEXT' },
      {
        name: 'decisionDeposito',
        label: 'Decisión depósito',
        type: 'SELECT',
        options: buildSelectOptions([
          'Devolver 100%',
          'Devolver parcial',
      ...
      { name: 'porcentajeDevolucion', label: 'Porcentaje devolución', type: 'NUMBER' },
      { name: 'montoDepositoOriginal', label: 'Monto depósito original', type: 'NUMBER' },
      { name: 'montoADevolver', label: 'Monto a devolver', type: 'NUMBER' },
      { name: 'montoARetener', label: 'Monto a retener', type: 'NUMBER' },
      { name: 'justificacionRetencion', label: 'Justificación retención', type: 'TEXT' },
      {
        name: 'aprobadoPorComercial',
        label: 'Aprobado por comercial',
        type: 'BOOLEAN',
      },
      { name: 'aprobadoPor', label: 'Aprobado por', type: 'TEXT' },
      { name: 'fechaAprobacion', label: 'Fecha aprobación', type: 'DATE' },
      { name: 'actaFirmadaCliente', label: 'Acta firmada cliente', type: 'BOOLEAN' },
      { name: 'documentoActaUrl', label: 'Documento acta URL', type: 'TEXT' },
    ],
  },
  {
    ...
    fields: [
      ...
      {
        name: 'condonacionSolicitada',
        label: 'Condonación solicitada',
        type: 'BOOLEAN',
      },
      { name: 'condonacionMotivo', label: 'Motivo condonación', type: 'TEXT' },
      {
        name: 'condonacionEstatus',
        label: 'Estatus condonación',
        type: 'SELECT',
        options: buildSelectOptions(['Pendiente', 'Aprobada', 'Rechazada']),
      },
      {
        name: 'diasHoldoverAcumulados',
        label: 'Días holdover acumulados',
        type: 'NUMBER',
      },
      { name: 'montoAcumuladoUsd', label: 'Monto acumulado USD', type: 'NUMBER' },
      { name: 'montoCobradoUsd', label: 'Monto cobrado USD', type: 'NUMBER' },
      {
        name: 'resolucion',
        label: 'Resolución',
        type: 'SELECT',
  ...
  {
    nameSingular: 'litigio',
    namePlural: 'litigios',
    labelSingular: 'Litigio / Extrajudicial',
    labelPlural: 'Litigios y Extrajudiciales',
    icon: 'IconGavel',
    skipNameField: true,
    labelIdentifierFieldName: 'referencia',
    fields: [
      ...
      { name: 'parqueNombre', label: 'Parque', type: 'TEXT' },
      { name: 'inquilinoNombre', label: 'Inquilino', type: 'TEXT' },
      { name: 'montoRiesgoUsd', label: 'Monto en riesgo USD', type: 'NUMBER' },
    ],
  },
  {
    ...
    fields: [
      ...
      },
      {
        ...
        label: 'Tipo de pago',
        type: 'SELECT',
        options: buildSelectOptions(['Interno', 'Externo']),
      },
      {
        name: 'beneficiario',
        label: 'Beneficiario',
      ...
      { name: 'folio', label: 'Folio del deal', type: 'TEXT' },
      { name: 'clienteNombre', label: 'Cliente', type: 'TEXT' },
      { name: 'leasingOfficer', label: 'Leasing Officer', type: 'TEXT' },
      {
        name: 'origenDeal',
        label: 'Origen del deal',
        type: 'SELECT',
        options: buildSelectOptions([
          'Directo',
          'Broker Top 10',
      ...
      {
        name: 'tipoContratoComision',
        label: 'Tipo contrato (comisión)',
        type: 'SELECT',
        options: buildSelectOptions(['Nuevo', 'Renovación']),
      },
      {
        name: 'estatusNaveComision',
        label: 'Estatus nave (comisión)',
        type: 'SELECT',
        options: buildSelectOptions([
          'Construida',
          'Por construir',
      ...
      {
        name: 'brokerTierSnapshot',
        label: 'Tier broker al cierre',
        type: 'SELECT',
        options: buildSelectOptions(['Directo', 'Top 10', 'No top 10']),
      },
      { name: 'rentaTotalContrato', label: 'Renta total contrato', type: 'NUMBER' },
      { name: 'pctAplicado', label: '% aplicado', type: 'NUMBER' },
      ...
      { name: 'aprobadoPor', label: 'Aprobado por', type: 'TEXT' },
      { name: 'fechaAprobacion', label: 'Fecha aprobación', type: 'DATE' },
      { name: 'ajusteMonto', label: 'Ajuste manual monto', type: 'NUMBER' },
      { name: 'motivoAjuste', label: 'Motivo ajuste', type: 'TEXT' },
      { name: 'fechaCierre', label: 'Fecha cierre deal', type: 'DATE' },
...
export const PARKS_RELATION_DEFINITIONS: RelationDefinition[] = [
  {
    objectNameSingular: 'broker',
    name: 'empresaBroker',
    label: 'Empresa de Brokers',
    targetObjectNameSingular: 'empresaBroker',
    targetFieldLabel: 'Brokers',
  ...
  {
    objectNameSingular: 'actaRestitucion',
    name: 'casoLegal',
    label: 'Caso Legal',
    targetObjectNameSingular: 'casoLegal',
    targetFieldLabel: 'Actas de Restitución',
    targetFieldIcon: 'IconDoorExit',
  },
  {
    objectNameSingular: 'actaRestitucion',
    name: 'inquilino',
